...
- **Python references are value-position aware** — a name used as a callback argument, dict key or value, list/set element, comprehension iterable/element/condition, assignment or augmented-assignment RHS, default parameter, decorator, `except`/`raise` target, type annotation, `await` operand, splat (`*a`/`**k`), `elif` condition, slice bound, `del` target, subscript index, or attribute read (`self._cache`, `cls._registry`, `C._x` — call or non-call) keeps its definition alive.
- **Local bindings do not shadow module-level names** — a reference inside a function that resolves to that function's parameter or local variable is not counted, so a local `_x` no longer hides a dead module-level `_x`.
- **Go and TypeScript locals are excluded** — `variable`/`constant` definitions (Go `var`/`const` specs, TS `variable_declarator`s) inside function, method, or closure bodies are locals and never appear in results; package-level declarations and TS class fields are kept.
- **Go and TypeScript references are value-position aware** — Go: call arguments, composite-literal values, assignment/short-var-declaration RHS, return values, selector operands and non-call selector reads (`pkg.X`), and type references in non-definition positions (parameter/result/field/var/const types, pointer/slice/map/channel elements, composite-literal types, type assertions, generic type args). TypeScript: call arguments, object-literal pair values, declarator values, assignment RHS, array elements, return values, member objects and non-call property reads, subscript objects/indices, `new` constructors, and type references inside type annotations and generic arguments. A definition is never captured as a reference to itself, so a genuinely unused type is still reported.
- **What is still not covered (all languages)** — references that only exist through framework or metaprogramming indirection are invisible: handlers invoked by the framework after decorator/registration setup (the decorated function itself is only "referenced" by the framework, not by source), reflection-based invocation, and pytest fixtures referenced by parameter name. Symbols reachable only these ways may be reported dead.
